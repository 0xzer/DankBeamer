...
module.exports = {
	...
	data: new SlashCommandBuilder()
		.setName('deposit')
		.setDescription("Deposit coins into your bank out of your pocket.")
	...
	async execute(interaction) {
        ...
        if (player.bank >= player.bankMax) {
            embed.setColor("Red");
            embed.setDescription(`You already have the max amount allowed in your bank!`);
        ...
        if (parsed > player.wallet) {
            embed.setColor("Red");
            embed.setDescription(`You're trying to deposit more than you've got in your wallet!`);
            return await interaction.reply({
                embeds: [embed]
            });
        };
        if (parsed+player.bankMax >= player.bankMax) parsed = (player.bankMax-player.bank);
        embed.setColor(invisibleEmbed);
        embed.addFields(
            {name: "Deposited", value: `${interaction.client.symbols.money} ${parsed.toLocaleString()}`},
        ...
        await interaction.client.database.addCoinsBank(interaction.user.id,parsed);
        await interaction.client.database.subtractCoins(interaction.user.id,parsed);
        return await interaction.reply({
            embeds: [embed]
        });
	},
